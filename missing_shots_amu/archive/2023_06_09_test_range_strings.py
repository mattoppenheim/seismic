...
test_range_data = [ ([18,1,121,94,0,120,121], [(0,1), (18, 18), (94, 94), (120, 121), (121,121)] ),
             ( [25, 7, 9], [(7, 7), (9, 9), (25, 25)] ),
             ( [1], [(1,1)] ),
             ( [], [] ) ]

test_missing_data = [( [0, 1, 3, 6], [2, 4, 5] ),
        ( [0, 0, 1, 1, 3], [2]), ([], []) ]

test_remove_multiples_data = [([], []), ([1, 2, 3], [1, 2, 3]),
        ( [1, 1, 2, 2, 3], [1, 2, 3]) ]

test_reverse_ranges_data =  [ ('50,25,23-20,9-6,3-1', '1-3,6-9,20-23,25,50'),
        ('', ''),
        ('1, 1', '1,1') ]
...
@pytest.mark.parametrize("test_list, expected", test_missing_data)
def test_find_missing(test_list, expected):
    assert find_missing(test_list) == expected


@pytest.mark.parametrize("test_list, expected", test_remove_multiples_data)
def test_remove_multiples(test_list, expected):
    assert remove_multiples(test_list) == expected


@pytest.mark.parametrize("test_list, expected", test_range_data)
def test_range_tuples(test_list, expected):
    assert range_tuples(test_list) == expected


@pytest.mark.parametrize("test_list, expected", test_reverse_ranges_data)
